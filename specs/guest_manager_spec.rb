...
class GuestManagerTest < MiniTest::Test
  ...
  def setup()
    ...
    guest_names.each do |guest|
      @my_guest_manager.add_guest(guest[0], guest[1], guest[2], guest[3], guest[4], guest[5])
    end
  end

  def test_can_add_guest()
    @empty_guest_manager.add_guest("Mark", "Jones", 1967, 10, 23, 6)
    mark_jones = Guest.new("Mark", "Jones", Date.new(1967, 10, 23), 6)
    assert(@empty_guest_manager.guests.include?(mark_jones))
  end

  def test_cant_duplicate_guest_details()
    assert_equal(false, @my_guest_manager.add_guest("Terry", "Marx", 1992, 3, 1, 0))
  ...
  def test_has_guest()
    mike_jones = Guest.new("Mike", "Jones", Date.new(1999, 2, 24), 9)
    assert(@my_guest_manager.has_guest(mike_jones))
  end

  def test_can_save_then_load()
    @my_guest_manager.save_guests()
    @my_guest_manager.add_guest("Tom", "Cat", 2001, 9, 23, 13)
    @my_guest_manager.load_guests()
    tom_cat = Guest.new("Tom", "Cat", Date.new(2001, 9, 23), 13)
